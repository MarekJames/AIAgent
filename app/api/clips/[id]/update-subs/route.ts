...
import { buildFFmpegFilter, type CropKF } from "@/src/services/framingService"
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    let usedSmartFraming = false

    try {
      if (clip.smartFramed && clip.cropMapJson) {
        log("framing:smart:start")
        const probe = await probeVideo(sourcePath)
        const cropMap = clip.cropMapJson as unknown as CropKF[]
        const filterExpr = buildFFmpegFilter(probe.width, probe.height, cropMap)
        ...
        usedSmartFraming = true
        log("framing:smart:done")
      }
    } catch (e: any) {
      logErr("framing:smart:error", e?.message || e)
    }

    if (!usedSmartFraming) {
      try {
        log("render:center:start", { start: clip.startSec, duration: clip.durationSec })
        ...
        log("render:center:done")
      } catch (e: any) {
        logErr("render:center:error", e?.message || e)
