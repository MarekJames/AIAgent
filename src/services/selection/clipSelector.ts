...
export function selectClips(
  ...
  for (const seg of candidates) {
    ...
    const variants = targets.map((t) =>
      evaluateForTarget(seg, t, options.videoEmbedding)
    );
...
function snapToSentence(seg: Segment, target: number): Segment {
  ...
  if (!isFinite(bestDelta)) {
    ...
    if (last) {
      bestEnd = Math.min(last.end + 3, start + maxAccept);
    }
  }
  if (bestEnd <= start + minAccept) {
    bestEnd = Math.min(start + minAccept + 2, start + maxAccept);
  }
...
function pillarSim(a: SelectedClip, b: SelectedClip): number {
  const va = [
    a.pillars.hook,
    a.pillars.watchability,
    a.pillars.visuals,
    a.pillars.safety,
    a.pillars.novelty,
    a.pillars.coherence,
    a.pillars.durationFit,
  ];
  const vb = [
    b.pillars.hook,
    b.pillars.watchability,
    b.pillars.visuals,
    b.pillars.safety,
    b.pillars.novelty,
    b.pillars.coherence,
    b.pillars.durationFit,
  ];
  return cosine(va, vb);
}
