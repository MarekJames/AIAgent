...
export function finalizeBestClips(
  inputs: RankInput[],
  maxClips: number = 5
): RankOutput[] {
  ...
  for (let i = 0; i < t120.length && out.length < q120; i++) {
    out.push(t120[i]);
  }
  for (let i = 0; i < t60.length && out.length < maxClips; i++) {
    out.push(t60[i]);
  }
  if (out.length < maxClips) {
    const rest = ranked.filter((r) => !out.find((x) => x.id === r.id));
    for (let i = 0; i < rest.length && out.length < maxClips; i++) {
      out.push(rest[i]);
    }
  }
