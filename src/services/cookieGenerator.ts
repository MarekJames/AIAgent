...
export async function getCookieFilePath(
  userId: string
): Promise<string | null> {
  ...
  await fs.writeFile(tempFile, user.youtubeCookies, {
    encoding: "utf-8",
    mode: 0o600,
  });
  ...
  if (user.youtubeCookiesCreatedAt) {
    const ageInDays =
      (Date.now() - user.youtubeCookiesCreatedAt.getTime()) /
      (1000 * 60 * 60 * 24);

    if (ageInDays > COOKIE_MAX_AGE_DAYS) {
      console.warn(
        `YouTube cookies for user ${userId} are ${Math.floor(
          ageInDays
        )} days old and may be expired`
      );
    } else {
      console.log(
        `Using YouTube cookies for user ${userId} (${Math.floor(
          ageInDays
        )} days old)`
      );
    }
