...
export function computeFeatures(
  ...
  const durationPenalty = durationFitPenalty(
    duration,
    targetDuration,
    minAccept,
    maxAccept
  ...
  const sentenceCompleteness = completenessScore(
    segment.transcript,
    segment.words
  );
...
function hookScore(words: Word[], seconds: number): number {
  ...
  for (const p of phrases) {
    if (firstWords.join(" ").includes(p)) {
      score = score + 1;
    }
  }
  const q =
    firstWords.includes("why") ||
    firstWords.includes("how") ||
    firstWords.includes("what")
      ? 0.3
      : 0;
