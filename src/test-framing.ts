...
async function createTestVideo(): Promise<string> {
  ...
  console.log(
    `[Test] Creating synthetic test video (1920x1080, ${TEST_DURATION}s)...`
  );
  console.log(
    `[Test] Simulating two people standing far apart with moving colored boxes...`
  );
  ...
  try {
    await execFileAsync(
      ...
      [
        ...
        "lavfi",
        "-i",
        `color=c=black:s=1920x1080:d=${TEST_DURATION}:r=25`,
        "-vf",
        `drawbox=x=200:y=200:w=300:h=600:color=blue:t=fill,drawbox=x=1400:y=200:w=300:h=600:color=red:t=fill,drawtext=text='Person 1':fontsize=40:fontcolor=white:x=250:y=400,drawtext=text='Person 2':fontsize=40:fontcolor=white:x=1450:y=400`,
        "-pix_fmt",
        "yuv420p",
        "-c:v",
        "libx264",
...
async function testFraming() {
  ...
  console.log(
    "Expected behavior: System should use z_min >= 1.0 to prevent impossible crops"
  );
  console.log();
  ...
  try {
    ...
    console.log(
      `[Test] Video info: ${videoInfo.width}x${
        videoInfo.height
    ...
    console.log(
      `  - targetW: ${targetW}px (9:16 from ${videoInfo.height}px height)`
    );
    console.log(
      `  - z_min_width: ${zMinWidth.toFixed(3)} (${targetW} / ${
        videoInfo.width
      })`
    );
    console.log(
      `  - z_min_height: ${zMinHeight.toFixed(3)} (always 1.0 for full height)`
    );
    ...
    if (!cropMap || cropMap.length === 0) {
      console.error(
        `[Test] ❌ FAILED: No crop map generated (no people detected?)`
      );
      process.exit(1);
    }
    ...
    if (zMin < zMinExpected - 0.001) {
      ...
      console.error(
        `  Expected z >= ${zMinExpected.toFixed(
          3
        )}, but got z_min = ${zMin.toFixed(3)}`
      );
      process.exit(1);
    }
    ...
    for (let i = 0; i < samplesToCheck; i++) {
      ...
      if (cropW > videoInfo.width || cropH > videoInfo.height) {
        console.error(
          `[Test] ❌ Invalid crop at t=${k.t.toFixed(
            2
          )}s: crop=(${cropW}x${cropH}) > video=(${videoInfo.width}x${
            videoInfo.height
          })`
        );
        allValid = false;
      ...
      if (
        k.x < 0 ||
        k.y < 0 ||
        k.x + cropW > videoInfo.width ||
        k.y + cropH > videoInfo.height
      ) {
        console.error(
          `[Test] ❌ Invalid crop position at t=${k.t.toFixed(2)}s: pos=(${
            k.x
          },${k.y}) size=(${cropW}x${cropH})`
        );
        allValid = false;
      }
    }
    ...
    try {
      const filter = buildFFmpegFilter(
        videoInfo.width,
        videoInfo.height,
        cropMap
      );
      ...
      console.error(
        `[Test] ❌ FAILED: FFmpeg filter generation failed:`,
        error
      );
      process.exit(1);
    }
    ...
    try {
      const filterExpr = buildFFmpegFilter(
        videoInfo.width,
        videoInfo.height,
        cropMap
      );
    ...
    console.log("Summary:");
    console.log(
      `  - Dimension-aware z_min working correctly (${zMinExpected.toFixed(3)})`
    );
