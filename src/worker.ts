  ...
  try {
    ...
    if (metadata.chapters.length > 0) {
      metadata.chapters.forEach((ch, i) => {
        console.log(
          `  Chapter ${i + 1}: ${ch.title} (${ch.startSec.toFixed(
            1
          )}s - ${ch.endSec.toFixed(1)}s)`
        );
      });
    }
    ...
    console.log(
      `Measured bitrate: ${br.kbps.toFixed(0)} kbps, size: ${(
        br.size /
        1024 /
        1024
      ).toFixed(1)} MB, duration: ${br.seconds.toFixed(1)}s`
    );
    ...
    if (br.kbps < minBitrate) {
      console.warn(
        `⚠️  Quality Gate: Source bitrate ${br.kbps.toFixed(
          0
        )} kbps is below recommended ${minBitrate} kbps for ${
          sourceInfo.height
        }p`
      );
    } else {
      console.log(
        `✓ Quality Gate: Source quality meets ${
          sourceInfo.height
        }p standards (${br.kbps.toFixed(0)} >= ${minBitrate} kbps)`
      );
    }
    ...
    if (ytVideoId) {
      ...
      if (comments.length > 0) {
        ...
        console.log(
          `Found ${
            commentHotspots.length
          } comment hotspots at: ${commentHotspots
            .map(
    ...
    console.log(
      `Selected top ${topCandidates.length} candidates for AI scoring (saving ${
        segments.length - topCandidates.length
      } GPT-4o calls)`
    );
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (isTopCandidate) {
        try {
          ...
          console.log(
            `  Candidate ${topCandidates.indexOf(seg) + 1}/${
              topCandidates.length
            }: AI overall score = ${scores.scores.overall}`
          );
        } catch (err) {
          console.error(
    ...
    console.log(
      `Selected ${ranked.length} clips: ${ranked
        .map((r) => `${r.tier}-tier`)
        .join(", ")}`
    );
    ...
    console.log(
      `Computing GLOBAL crop for entire video (consistent across all clips)`
    );
    let globalCrop = null;
    try {
      const { computeGlobalStaticCrop } = await import(
      ...
      if (globalCrop) {
        console.log(
          `✓ Global crop computed: ${globalCrop.cropW}x${globalCrop.cropH} @ (${globalCrop.cropX},${globalCrop.cropY})`
        );
        ...
        console.log(
          `⚠️  No persons detected for global crop, will use per-segment framing`
        );
      }
    } catch (err) {
    ...
    const processSegment = async (segment: any, i: number) => {
      ...
      console.log(
        `Computing static person-centered framing for clip ${i + 1}/${
    ...
    const summary = {
      ...
      tierBreakdown: selectedSegments.reduce((acc, seg) => {
